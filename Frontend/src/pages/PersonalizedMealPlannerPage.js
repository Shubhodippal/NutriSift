...
function PersonalizedMealPlannerPage() {
  ...
  const handleSaveRecipe = async (recipe) => {
    try {
      ...
      // Convert origin object to string if it's an object
      const origin = typeof recipe.origin === 'object' 
        ? `${recipe.origin.country}, ${recipe.origin.region}` 
        : (recipe.origin || '');
      ...
      const recipeData = {
        ...
        course: course,
        cuisine: cuisine,
        prompt: recipe.userInput || 'Meal plan generated by NutriSift',
      };
    ...
    try {
      ...
      groceryItems.forEach(newItem => {
        ...
        if (existingIndex >= 0) {
          ...
          // Add this check to ensure meals exists
